...
int main(){
	...
	int inputPipes = nodeSystemAddPipe("IN",NODE_PIPE_IN   ,NODE_IN_UNIT,1,NULL);
	...
	node_in_unit rdata;
    node_out_unit wdata = 0;
	double buffer = 0;

	nodeSystemWait();
	while(!nodeSystemLoop()){
		//read input
		int ret = nodeSystemRead(inputPipes,&rdata);
		
		//write sub result
		if(ret >= 0){

			buffer += rdata * (nodeSystemGetPeriod()/1000.0);
			wdata = buffer;
